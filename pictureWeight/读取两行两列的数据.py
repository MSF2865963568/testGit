...
def getData(csv_path):
    with open(csv_path) as csv_file:
        ...
        # 读取除首行之后每一行的第2列数据，并将其加入到数组price之中
        for r in row:
...
if __name__ == '__main__':
    path_data = r'../Wresult\Ytrain\Ytrain_Conv2d0.csv'
    # 自变量
    # 读取csv文件数据读取为numpy.array型数据
    # data_array = np.genfromtxt(path_data)[1:]
    data_array = np.genfromtxt(path_data, skip_header=1, usecols=(1), delimiter=',')
    # print(data_array)
    # data_array = np.arange(-4, 5, 0.1)
    print(data_array)
